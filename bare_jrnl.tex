...
To capture the constraints imposed by limited onboard processing resources, we adopt a fluid-based overload model with probabilistic frame dropping \cite{misra2000fluid}. 
Each processing stage $j$ is assigned a nominal capacity $C_j$, and $r_j$ denotes the aggregate traffic rate reaching that stage after upstream filtering and frame dropping. 
No resource-induced frame loss occurs when $r_j \leq C_j$. 
When $r_j>C_j$, each incoming frame is independently dropped with probability
\[
p_j^{\mathrm{drop}} = 1-\frac{C_j}{r_j},
\]
such that the expected output rate does not exceed $C_j$.

The nominal processing capacities of the onboard router, OSMD unit, and MTC unit are set to $1000~\mathrm{Mb/s}$, $500~\mathrm{Mb/s}$, and $50~\mathrm{Mb/s}$, respectively. 
The execution unit has a capacity of $500~\mathrm{Mb/s}$ when shadow execution is disabled, which is reduced to $250~\mathrm{Mb/s}$ when heterogeneous shadow execution and output verification are enabled. 
